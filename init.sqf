...
FUNC(debug_addActions) = {
	...
	player addAction ["<t color='#0044FF'>Debug</t>", {if (isNil "cre_handle_debug") then {cre_handle_debug = scriptNull}; terminate cre_handle_debug; cre_handle_debug = [] spawn compile preprocessFileLineNumbers "dev\debug.sqf"}, nil, 10, false, false, "lockTargets"];
	...
	player addAction ["<t color='#00DD00'>Init Client + Server</t>", {
		isNil {
			...
			call compile preprocessFileLineNumbers "scripts\gamemode\fn_gm_preInit.sqf";
