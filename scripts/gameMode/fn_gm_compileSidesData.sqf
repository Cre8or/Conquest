...
private ["_muzzleRecoilMulCache"];
{
	...
	_loadoutsCache = createHashMap;
	{
		...
		if (_role == MACRO_ENUM_ROLE_INVALID) then {
			diag_log format ["[CONQUEST] ERROR: Invalid role enumeration type %1!", _role];
			continue;
		};

		if (_role in _loadoutsCache) then {
			diag_log format ["[CONQUEST] ERROR: Role enumeration type ""%1"" is defined multiple times!", _role];
			continue;
		};

		// Compatibility with ACE extended loadouts
		if (count _loadout == 2) then {
			_loadout = _loadout param [0, [], [[]], 10]; // Loadout arrays contain 10 entries
		};

		if (_loadout isEqualTo []) then {
			diag_log format ["[CONQUEST] ERROR: Role enumeration type %1 has no loadout!", _role];
			continue;
		};
	...
	_vehTypesIndexCache = createHashMap;
	{
		...
		_definitionsXCopy = [];
		{
			...
			// Enforce class validity (useful after mod updates/removals)
	...
	_muzzleRecoilMulCache = createHashMap;
	{
		...
		if (_muzzle in _muzzleRecoilMulCache) then {
			diag_log format ["[CONQUEST] ERROR: Recoil multiplier for muzzle ""%1"" is defined multiple times!", _role];
			continue;
		};
