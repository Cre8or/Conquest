// Medical action: revive unconscious friendlies
...
if (
	...
	and {_teamMates findIf {
		_x getVariable [QGVAR(isUnconscious), false]
		and {_x distanceSqr _player < _c_maxDistSqr}
	} >= 0}
) then {

	private _key       = MACRO_ENUM_TUTORIALHINT_MEDICAL_REVIVEFRIENDLY;
	private _hintShown = GVAR(gm_sys_tutorialHints_hashmap) getOrDefault [_key, false];

	if (!_hintShown) then {
		GVAR(gm_sys_tutorialHints_activeHint) = MACRO_ENUM_TUTORIALHINT_MEDICAL_REVIVEFRIENDLY;
