// Update the faction menu
case "ui_update_faction": {
	_eventExists = true;
	...
	// On a two-sided setup, swap the panels around such that the middle one remains free
	...
	// Initial setup
	if !(_spawnMenu getVariable [QGVAR(menuFaction_isInit), false]) then {
		_spawnMenu setVariable [QGVAR(menuFaction_isInit), true];
		...
		// Fill out the controls with actual data (flag textures, side names, etc.)
		{
			...
			[_sideLeft,   MACRO_IDC_SM_FACTION_PLAYERS_LEFT_LISTBOX,   MACRO_IDC_SM_FACTION_FLAG_LEFT_PICTURE,   MACRO_IDC_SM_FACTION_NAME_LEFT_TEXT],
			[_sideMiddle, MACRO_IDC_SM_FACTION_PLAYERS_MIDDLE_LISTBOX, MACRO_IDC_SM_FACTION_FLAG_MIDDLE_PICTURE, MACRO_IDC_SM_FACTION_NAME_MIDDLE_TEXT],
			[_sideRight,  MACRO_IDC_SM_FACTION_PLAYERS_RIGHT_LISTBOX,  MACRO_IDC_SM_FACTION_FLAG_RIGHT_PICTURE,  MACRO_IDC_SM_FACTION_NAME_RIGHT_TEXT]
		];

		// Save the resulting array of controls to be updated
		_spawnMenu setVariable [QGVAR(menuFaction_ctrls), _ctrls];
	};
		...
		if (_sideLeft == sideEmpty) then {
			{
				...
				MACRO_IDC_SM_FACTION_FLAG_LEFT_PICTURE,
				MACRO_IDC_SM_FACTION_FLAG_LEFT_GRADIENT,
				MACRO_IDC_SM_FACTION_FLAG_LEFT_FILL,
				MACRO_IDC_SM_FACTION_FLAG_LEFT_SEPARATOR,
				MACRO_IDC_SM_FACTION_NAME_LEFT_TEXT,
				MACRO_IDC_SM_FACTION_TICKETS_LEFT_TEXT,
				MACRO_IDC_SM_FACTION_PLAYERS_LEFT_BACKGROUND,
				MACRO_IDC_SM_FACTION_PLAYERS_LEFT_OUTLINE,
				MACRO_IDC_SM_FACTION_PLAYERS_LEFT_LISTBOX,
				MACRO_IDC_SM_FACTION_JOIN_LEFT_FRAME,
				MACRO_IDC_SM_FACTION_JOIN_LEFT_BUTTON
			];
		};
		...
		if (_sideMiddle == sideEmpty) then {
			{
				...
				MACRO_IDC_SM_FACTION_FLAG_MIDDLE_PICTURE,
				MACRO_IDC_SM_FACTION_FLAG_MIDDLE_GRADIENT,
				MACRO_IDC_SM_FACTION_FLAG_MIDDLE_FILL,
				MACRO_IDC_SM_FACTION_FLAG_MIDDLE_SEPARATOR,
				MACRO_IDC_SM_FACTION_NAME_MIDDLE_TEXT,
				MACRO_IDC_SM_FACTION_TICKETS_MIDDLE_TEXT,
				MACRO_IDC_SM_FACTION_PLAYERS_MIDDLE_BACKGROUND,
				MACRO_IDC_SM_FACTION_PLAYERS_MIDDLE_OUTLINE,
				MACRO_IDC_SM_FACTION_PLAYERS_MIDDLE_LISTBOX,
				MACRO_IDC_SM_FACTION_JOIN_MIDDLE_FRAME,
				MACRO_IDC_SM_FACTION_JOIN_MIDDLE_BUTTON
			];
		};
		...
		if (_sideRight == sideEmpty) then {
			{
				...
				MACRO_IDC_SM_FACTION_FLAG_RIGHT_PICTURE,
				MACRO_IDC_SM_FACTION_FLAG_RIGHT_GRADIENT,
				MACRO_IDC_SM_FACTION_FLAG_RIGHT_FILL,
				MACRO_IDC_SM_FACTION_FLAG_RIGHT_SEPARATOR,
				MACRO_IDC_SM_FACTION_NAME_RIGHT_TEXT,
				MACRO_IDC_SM_FACTION_TICKETS_RIGHT_TEXT,
				MACRO_IDC_SM_FACTION_PLAYERS_RIGHT_BACKGROUND,
				MACRO_IDC_SM_FACTION_PLAYERS_RIGHT_OUTLINE,
				MACRO_IDC_SM_FACTION_PLAYERS_RIGHT_LISTBOX,
				MACRO_IDC_SM_FACTION_JOIN_RIGHT_FRAME,
				MACRO_IDC_SM_FACTION_JOIN_RIGHT_BUTTON
			];
		};
	};
	...
	private ["_ctrlX", "_ticketsX"];
	{
		...
		[_sideLeft,   MACRO_IDC_SM_FACTION_TICKETS_LEFT_TEXT],
		[_sideMiddle, MACRO_IDC_SM_FACTION_TICKETS_MIDDLE_TEXT],
		[_sideRight,  MACRO_IDC_SM_FACTION_TICKETS_RIGHT_TEXT]
	];
	...
	private _alive = [player] call FUNC(unit_isAlive);
	{
		...
		if (!_isPlayable) then {
			_ctrlButton ctrlSetText toUpper (LSTRING(ui_spawnMenu_faction_unavailable) call BIS_fnc_localize);
		} else {
			if (_isSelected) then {
				_ctrlButton ctrlSetText toUpper (LSTRING(ui_spawnMenu_faction_selected) call BIS_fnc_localize);
			} else {
				_ctrlButton ctrlSetText toUpper (LSTRING(ui_spawnMenu_faction_clicktoselect) call BIS_fnc_localize);
			};
		};

	} forEach [
		[_sideLeft,	  MACRO_IDC_SM_FACTION_JOIN_LEFT_FRAME,   MACRO_IDC_SM_FACTION_JOIN_LEFT_BUTTON],
		[_sideMiddle, MACRO_IDC_SM_FACTION_JOIN_MIDDLE_FRAME, MACRO_IDC_SM_FACTION_JOIN_MIDDLE_BUTTON],
		[_sideRight,  MACRO_IDC_SM_FACTION_JOIN_RIGHT_FRAME,  MACRO_IDC_SM_FACTION_JOIN_RIGHT_BUTTON]
	];

	// Update the names lists
	private _ctrls = _spawnMenu getVariable [QGVAR(menuFaction_ctrls), []];
	...
	private ["_curSide", "_curSideIndex", "_ctrlListBox", "_unit", "_index"];
	{
		...
		// Add the AI units from this side
		{
			...
			if (_x # MACRO_INDEX_AIIDENTITY_SIDEINDEX == _curSideIndex) then {
				_unit = missionNamespace getVariable [format [QGVAR(AIUnit_%1), _forEachIndex], objNull];

				_ctrlListBox lnbAddRow ["AI", _x # MACRO_INDEX_AIIDENTITY_NAME];
