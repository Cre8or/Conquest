/* --------------------------------------------------------------------------------------------------------------------
	...
	Description:
		[LE]
		Increases the danger level of the edge between the two provided AI nodes.
	...
	Arguments:
		...
		3:	<NUMBER>	The danger level increment (optional, default: -1)
	Returns:
		(nothing)
-------------------------------------------------------------------------------------------------------------------- */
...
if (isNull _nodeA or {isNull _nodeB}) exitWith {systemChat format ["Invalid nodes: %1 / %2", _nodeA, _nodeB]};

private _strAB = format [QGVAR(dangerLevel_%1), _nodeIDB];
private _strBA = format [QGVAR(dangerLevel_%1), _nodeIDA];
...
systemChat format ["Danger level %1-%2 (%3): %4 -> %5", _nodeIDA, _nodeIDB, ["inf", "veh"] select _isVehNodeMesh, _dangerLevelA, _nodeA getVariable [_strAB, 0]];
